<x-app-layout>
    ...
    <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 mb-6">Buildings</h2>

    <div class="flex flex-wrap gap-6 mb-8 px-4 mt-8"> <!-- Add top margin here to create space -->
        @foreach($infrastructures as $infra)
            <form method="post" action="{{ route('client.government-infrastructure.update', $infra) }}" class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow text-center flex-1 max-w-xs">
                @method('patch')
                @csrf
    ...
    <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 mb-6">Unbuilt Buildings</h2>

    <div class="flex flex-wrap gap-6 mb-8 px-4 mt-8"> <!-- Add top margin here to create space -->
        @foreach($unbuiltInfrastructures as $infra)
            <form method="post" action="{{ route('client.government-infrastructure.store', $infra) }}" class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow text-center flex-1 max-w-xs">
                @csrf
